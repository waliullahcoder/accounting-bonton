    ...
    <div class="row g-3">
        ...
        <div class="col-sm-6">
            ...
            <select class="form-select select" name="software_type" id="software_type"
                ...
                <option value=""></option>
                <option value="Apppro ERP" {{ old('software_type') == 'Apppro ERP' ? 'selected' : '' }}>Apppro ERP</option>
                <option value="Sales Tracker" {{ old('software_type') == 'Sales Tracker' ? 'selected' : '' }}>Sales Tracker
                </option>
                <option value="Membership" {{ old('software_type') == 'Membership' ? 'selected' : '' }}>Membership</option>
                <option value="Luckydraw" {{ old('software_type') == 'Luckydraw' ? 'selected' : '' }}>Luckydraw</option>
                <option value="Customized Software" {{ old('software_type') == 'Customized Software' ? 'selected' : '' }}>
                    Customized Software</option>
                <option value="Ecommerce" {{ old('software_type') == 'Ecommerce' ? 'selected' : '' }}>Ecommerce</option>
                <option value="Others" {{ old('software_type') == 'Others' ? 'selected' : '' }}>Others</option>
            </select>
        </div>
        <div class="col-sm-6">
            ...
            <select name="pay_type" id="pay_type" class="form-select" data-placeholder="Select Pay Type" required>
                ...
                <option value="Quarterly" {{ old('pay_type') == 'Quarterly' ? 'selected' : '' }}>Quarterly</option>
                <option value="Tri-Yearly" {{ old('pay_type') == 'Tri-Yearly' ? 'selected' : '' }}>Tri-Yearly</option>
                <option value="Half-Yearly" {{ old('pay_type') == 'Half-Yearly' ? 'selected' : '' }}>Half-Yearly</option>
                <option value="Yearly" {{ old('pay_type') == 'Yearly' ? 'selected' : '' }}>Yearly</option>
            </select>
        </div>
        <div class="col-sm-6">
            <label for="month" class="form-label"><b>Months <span class="text-danger">*</span></b></label>
            <select name="month[]" id="month" class="form-select select" data-placeholder="Select Months" multiple
                required>
                @for ($i = 1; $i <= 12; $i++)
                    <option value="{{ date('F', mktime(0, 0, 0, $i, 1)) }}"
                        {{ (is_array(request('month')) && in_array(date('F', mktime(0, 0, 0, $i, 1)), request('month'))) || is_null(request('month')) ? 'selected' : '' }}>
        ...
        <div class="col-sm-6">
            ...
            <input type="number" class="form-control" id="service_charge" name="service_charge"
                value="{{ old('service_charge') }}" placeholder="Service Charge" required>
        </div>
    </div>
@endsection
