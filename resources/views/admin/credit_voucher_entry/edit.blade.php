...
@section('content')
    <div class="row g-3">
        <div class="col-lg-4 col-sm-6">
            ...
            <select name="project_id" id="project_id" class="form-select select" data-placeholder="select Project Name">
                ...
                @foreach ($projects as $project)
                    <option value="{{ $project->id }}" {{ $project->id == $data->project_id ? 'selected' : '' }}>
                        {{ $project->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="debit_head" id="debit_head" class="form-select select" data-placeholder="select Account Name"
                ...
                @foreach ($debitCoas as $item)
                    <option value="{{ $item->id }}"
                        {{ old('debit_head', $data->coa_setup_id) == $item->id ? 'selected' : '' }}>
        ...
        <div class="col-lg-4 col-sm-6">
            <label for="voucher_date" class="form-label"><b>Date <span class="text-danger">*</span></b></label>
            <input type="text" class="form-control date_picker" name="voucher_date" id="voucher_date"
                value="{{ date('d-m-Y', strtotime(old('voucher_date', $data->voucher_date))) }}" placeholder="Date"
                required>
        </div>
        <div class="col-lg-4 col-sm-6">
        ...
        <div class="col-lg-12 col-sm-12">
            ...
            <textarea class="form-control narration" id="narration" name="narration" rows="1" spellcheck="false"
                placeholder="Remarks">{{ old('narration', $data->narration) }}</textarea>
        </div>
        <div class="col-12">
                    ...
                    <tbody id="tbody">
                        @foreach ($creditEntries as $item)
                            <tr>
                                ...
                                <td class="py-1">
                                    <input type="number" class="credit text-end form-control input-sm"
                                        name="credit_amount[{{ $item->coa_setup_id }}]" oninput="calculate()"
                        ...
                        <tr>
                            ...
                            <td class="py-1">
                                <input type="number" step="any" class="totalCredit text-end form-control input-sm"
                                    ...
                                    value="{{ $creditEntries->sum('credit_amount') }}" readonly>
                            </td>
                            <th></th>
                        </tr>
                    </tfoot>
