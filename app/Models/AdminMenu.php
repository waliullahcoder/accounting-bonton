...
class AdminMenu extends Model
{
    ...
    {
        $query->whereNull('parent_id')->with(['actions', 'children']);
    }

    public function children()
    {
        return $this->hasMany(AdminMenu::class, 'parent_id')->orderBy('order', 'asc')->where('status', 1)->with(['actions', 'children', 'permission']);
    }
