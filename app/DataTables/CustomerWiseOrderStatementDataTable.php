...
class CustomerWiseOrderStatementDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('total_count', function ($row) use ($year, $month) {
                ...
                $total = CustomerBookings::where('date', '>=', $start_date)->where('date', '<=', $end_date)->where('customer_id', $row->id)->sum('qty');
            ...
            ->addColumn('one', function ($row) use ($month, $year) {
                ...
                $total = CustomerBookings::where('date', $date)->where('customer_id', $row->id)->sum('qty');
                return $total > 0 ? $total : '-';
            })
            ...
            ->parameters([
                ...
                'drawCallback' => 'function() {
                    ...
                    data.forEach(function(item, index){
                        total_all = item.total_total_count;
    ...
    {
        return 'customer_wise_order_statement_' . date('d_m_Y_h_i_s_A');
    }
}
